...
public class MemManager{
    ...
    MemManager( int pbuffer_in , int hbits_in ){
        ...
        if ( hbits > 0 ){
            ...
            queue = new int[Machine.NumPhysPages - buffersize];
            for (int i = 0; i < queue.length; ++i){
                queue[i] = i + buffersize; //test 7 buffer size is 0 so does not matter.
            }
    ...
    int makeFreeFrame() {
        ...
        if (policy == FIFO) {
            ...
            //selecting based on queue counter
            victim = queue[queueCounter];
            queueCounter = (queueCounter + 1) % queue.length;
        }
        else if (policy == LRU) {
            for (int i = 0; i < history.length; i++) {
                int frame = (counter + i) % Machine.NumPhysPages;

                //checking if the frame has the lowest history value
                if (history[frame] < lowestValue && (!coreOwners[frame].use || !coreOwners[frame].valid)) {
                    lowestValue = history[frame];
                    victim = frame;
                }
            }

            //updating the counter to start from the next frame in the future
            counter = (victim + 1) % Machine.NumPhysPages;
        }
        else if (policy == ESC) {

            int originalQueueCounter = queueCounter;
            boolean first01 = false;
            int tempCounter = 0;

            do{
                int victim_frame = queue[queueCounter];
                TranslationEntry entry = coreOwners[victim_frame];

                if(entry.valid){
                    if(!entry.use && !entry.dirty){ //00
                        queueCounter = (queueCounter + 1)%queue.length;
                        return victim_frame;
                    }
                    else if(!entry.use && !first01){ //01
                        tempCounter = queueCounter;
                        first01 = true;
                    }
                    else if(entry.use){
                        entry.use = false;
                ...
                queueCounter = (queueCounter + 1)%queue.length;
            }while(queueCounter != originalQueueCounter);
            ...
            queueCounter = (tempCounter + 1)%queue.length;
            return queue[tempCounter];
        }
        return victim;
    }
/*
 ...
 */
    ...
    void recordHistory( int arg ){
        ...
        for (int i = 0; i < history.length; ++i) {
            history[i] >>= 1; //right shifting the history by 1

            if (coreOwners[i] != null && coreOwners[i].use) {
                history[i] |= lruUseBit; //setting the most significant bit if the page was used
                coreOwners[i].use = false; //resetting the used bit
            }
        }
    }
    ...
    void pageOut(int physFrame)
    {
        ...
        // It may or may not have been changed by the paging scheme, so just for good measure.
        victim_te.valid = false;
